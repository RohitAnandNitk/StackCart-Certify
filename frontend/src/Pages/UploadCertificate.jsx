...
function UploadCertificate() {
  ...
  const { createId, loading, certificateId, setCertificateId , certificateUploadCheck , uploadCertificate } =
    useCertificateStore();
...
//   }, [watchedFields]);
useEffect(() => {
  const fields = ["certificateId", "certificate"];
  ...
  const filledFields = fields.filter((field) => {
    ...
    if (field === "certificate" && value instanceof FileList) {
      return value.length > 0; // at least one file uploaded
    }
...
//     };
//     await uploadCertificate(formattedData);
//   };
  // In UploadCertificate.jsx

const onSubmit = async (data) => {
  // Create a new FormData object
  const formData = new FormData();

  // Append the certificate ID
  formData.append('certificateId', data.certificateId);

  // Append the file itself. data.certificate is a FileList, so we take the first file.
  // The key 'certificate' MUST match what your backend middleware (Multer) expects.
  if (data.certificate && data.certificate.length > 0) {
    formData.append('certificate', data.certificate[0]);
  }

  // Pass the entire formData object to your store action
  ...
  const formFields = [
    {
      ...
      icon: IdCard,
      validation: { required: "Certificate ID is required" },
    },
    {
      name: "certificate",
      ...
      icon: IdCard,
      validation: { required: "Certificate image is required" },
    },
          ...
          <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-green-50 to-blue-50 px-4 py-30">
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-12 shadow-2xl border border-white/20 max-w-2xl w-full text-center transform animate-in fade-in slide-in-from-bottom-10 duration-700">
              ...
              <h2 className="text-3xl font-bold bg-gradient-to-r from-green-600 to-blue-600 bg-clip-text text-transparent mb-4">
                Certificate Upload Successfully! 🎉
              </h2>

              <p className="text-gray-600 mb-8">
                Your new certificate has been generated
              </p>
          ...
          <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 px-4 py-30">
            ...
            <div className="text-center mb-12 animate-in fade-in slide-in-from-top duration-700">
              ...
              <p className="text-gray-600 text-lg">
                Fill in the details to Upload your certificate 
              </p>
            </div>
